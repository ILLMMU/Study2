---
title             : "Light exposure behaviors predict emotion, cognition and sleep quality"
shorttitle        : "Light exposure behaviors"
...
#This chunck folds the code for overall environment control
...
#Exclusion and inclusion crietaia
LB <- data %>% 
...
## Recoding LB$rec_LEBA1
LB$rec_LEBA1 <- as.character(LB$rec_LEBA1)
LB$rec_LEBA1[LB$rec_LEBA1 == "1"] <- "5"
LB$rec_LEBA1[LB$rec_LEBA1 == "2"] <- "4"
LB$rec_LEBA1[LB$rec_LEBA1 == "3"] <- "3"
LB$rec_LEBA1[LB$rec_LEBA1 == "4"] <- "2"
LB$rec_LEBA1[LB$rec_LEBA1 == "5"] <- "1"
LB$rec_LEBA1 <- as.numeric(LB$rec_LEBA1)

## Recoding LB$rec_LEBA2
LB$rec_LEBA2 <- as.character(LB$rec_LEBA2)
LB$rec_LEBA2[LB$rec_LEBA2 == "1"] <- "5"
LB$rec_LEBA2[LB$rec_LEBA2 == "2"] <- "4"
LB$rec_LEBA2[LB$rec_LEBA2 == "3"] <- "3"
LB$rec_LEBA2[LB$rec_LEBA2 == "4"] <- "2"
LB$rec_LEBA2[LB$rec_LEBA2 == "5"] <- "1"
LB$rec_LEBA2 <- as.numeric(LB$rec_LEBA2)
...
LB<- LB %>% 
  rowwise() %>% 
  mutate(LEBA.F1 = sum(rec_LEBA1:rec_LEBA3))

LB<- LB %>% 
  rowwise() %>% 
  mutate(LEBA.F2 = sum(rec_LEBA4:rec_LEBA9))

LB<- LB %>% 
  rowwise() %>% 
  mutate(LEBA.F3 = sum(rec_LEBA10: rec_LEBA12))

LB<- LB %>% 
  rowwise() %>% 
  mutate(LEBA.F4 = sum(rec_LEBA15, rec_LEBA16, rec_LEBA18))

LB<- LB %>% 
  rowwise() %>% 
  mutate(LEBA.F5 = sum(rec_LEBA19, rec_LEBA20, rec_LEBA22))
...
Descriptives <- LB %>% 
  dplyr::select( Age, Gender, Religion, Ethnicity, Marital_Status,
                ...
                HLEA_lx,MEQ_Total,MEQ_Chronotype, LEBA.F1:LEBA.F5, Num_of_Coffee_Weekdays, Num_of_coffee_Weekend, Work_Shift, KSS_Alertness, OLS_Concentration_rec, OLS_Memory_rec)
...
Descriptives$Trouble_Concentration <- as.numeric(Descriptives$Trouble_Concentration)

Descriptives$Trouble_in_Memory <- as.numeric(Descriptives$Trouble_in_Memory)
...
descriptives <- clear.labels(Descriptives)

descriptives %>%
  tbl_summary(
    by=Gender,
    type =  list(Trouble_in_Memory ~  "continuous", Trouble_Concentration ~  "continuous"),
...
modify_header(label ~ "**Variable**") %>% 
  ...
  modify_caption("Demographic Characteristics of Participants (n=301).") -> desc_table 
```
...
names(LEBA_stem)[names(LEBA_stem) == "LEBA2"] <- "LEBA03"
names(LEBA_stem)[names(LEBA_stem) == "LEBA3"] <- "LEBA02"
...
leba_vars_labels <- leba_vars_labels[c(1,3,2,4:18),]
...
names(LEBA)[names(LEBA) == "rec_LEBA2"] <- "LEBA03"
names(LEBA)[names(LEBA) == "rec_LEBA3"] <- "LEBA02"
...
LEBA <- LEBA %>% 
  dplyr::select(LEBA01, LEBA02, LEBA03, LEBA04:LEBA18)
...
# histogram and density plots
  ...
  gt::tab_row_group(
    label = "B1: Wearing blue light filters",
    rows = 01:03) %>%
 gt::tab_row_group(
   label = "B2: Spending time outdoors",
   rows = 04:09) %>% 
  gt::tab_row_group(
    label = "B3: Using phone and smartwatch in bed",
    rows = 10:12) %>% 
  gt::tab_row_group(
    label = "B4: Controlling environmental light before bedtime",
    rows = 13:15) %>% 
  gt::tab_row_group(
    label = "B5: Using light in the morning and during daytime",
    rows = 16:18)%>%
  gt::row_group_order(
    groups = c("B1: Wearing blue light filters", "B2: Spending time outdoors",
               "B3: Using phone and smartwatch in bed", "B4: Controlling environmental light before bedtime",
               "B5: Using light in the morning and during daytime" ))%>%
  tab_options(
    table.font.size = px(14L)) %>% 
  cols_width(
    Stem ~ px(200)) %>% 
  gt::gtsave("Figures/LEBA.png",vwidth = 1000)
```
...
knitr::include_graphics('Figures/Figure 4.png', dpi =300)
...
names(LB_1)[names(LB_1) == "rec_LEBA2"] <- "LEBA_F1_item3"
names(LB_1)[names(LB_1) == "rec_LEBA3"] <- "LEBA_F1_item2"
...
structural_model_1 <- relationships(
  ...
  paths(from = c("PSQ", "SE"), to = c("PA","NegA"))
)
...
# names(LB_2)[names(LB_2) == "rec_LEBA2"] <- "LEBA_F1_item3"
names(LB_2)[names(LB_2) == "rec_LEBA3"] <- "LEBA_F1_item2"
...
# Round 2 PLS excluding  MEQ items 6, 10,16,12 and LEBA LEBA item2 & item4. All loading is now above .40
...
measurment_model_2 <- constructs(
  ...
  composite("PSQ", multi_items("PSQ", 1:5)),
  composite("SE", multi_items("Sleep_efficieny", 1:2)),
  composite("L1", multi_items("LEBA_F1_item", 1:2)),
...
structural_model_2 <- relationships(
  paths(from = c("L1", "L2","L3","L4","L5"), to = c("PSQ","SE", "PA", "NegA", "Memory","Concentration", "PT", "MA", "RT", "RI")) ,
  paths(from = c("PT", "MA", "RT", "RI"), to = c( "Memory","Concentration")) ,
  paths(from = c( "PA","NegA", "PSQ", "SE"), to = c("Memory", "Concentration")),
  paths(from = c("PA","NegA", "PT", "MA", "RT", "RI"), to = c("PSQ", "SE")),
  paths(from = c("PSQ", "SE"), to = c("PA","NegA"))
)
...
measurment_table2 <- as.data.frame(matrix("",nrow=60,ncol=5))
measurment_table_names <- c("Constructs","Factor Loading","Cronbach's alpha","CR","AVE")
...
measurment_table2[11,2] <- mm2_factor_loading_L2_item1
measurment_table2[12,2] <- mm2_factor_loading_L2_item2
measurment_table2[13,2] <- mm2_factor_loading_L2_item3
measurment_table2[14,2] <- mm2_factor_loading_L2_item4
measurment_table2[15,2] <- mm2_factor_loading_L2_item5

measurment_table2[16,2] <- mm2_factor_loading_L3_item1
measurment_table2[17,2]<- mm2_factor_loading_L3_item2
measurment_table2[18,2]<- mm2_factor_loading_L3_item3

measurment_table2[19,2]<- mm2_factor_loading_L4_item1
measurment_table2[20,2]<- mm2_factor_loading_L4_item2
measurment_table2[21,2]<- mm2_factor_loading_L4_item3

measurment_table2[22,2]<- mm2_factor_loading_L5_item1
measurment_table2[23,2]<- mm2_factor_loading_L5_item2
measurment_table2[24,2]<- mm2_factor_loading_L5_item3

measurment_table2[25,2] <-mm2_factor_loading_MEQ_F1_item1
measurment_table2[26,2]<- mm2_factor_loading_MEQ_F1_item2
measurment_table2[27,2]<- mm2_factor_loading_MEQ_F1_item3
measurment_table2[28,2]<- mm2_factor_loading_MEQ_F1_item4
measurment_table2[29,2]<- mm2_factor_loading_MEQ_F1_item5
measurment_table2[30,2]<- mm2_factor_loading_MEQ_F1_item6

measurment_table2[31,2]<- mm2_factor_loading_MEQ_F2_item1
measurment_table2[32,2]<- mm2_factor_loading_MEQ_F2_item2
measurment_table2[33,2]<- mm2_factor_loading_MEQ_F2_item3

measurment_table2[34,2]<- mm2_factor_loading_MEQ_F3_item1
measurment_table2[35,2]<- mm2_factor_loading_MEQ_F3_item2
measurment_table2[36,2]<- mm2_factor_loading_MEQ_F3_item3
measurment_table2[37,2]<- mm2_factor_loading_MEQ_F3_item4

measurment_table2[38,2]<- mm2_factor_loading_MEQ_F4_item1
measurment_table2[39,2]<- mm2_factor_loading_MEQ_F4_item2

measurment_table2[40,2] <- mm2_factor_loading_PA_item1 
measurment_table2[41,2]<- mm2_factor_loading_PA_item2
measurment_table2[42,2]<- mm2_factor_loading_PA_item3
measurment_table2[43,2]<- mm2_factor_loading_PA_item4
measurment_table2[44,2]<- mm2_factor_loading_PA_item5
measurment_table2[45,2]<- mm2_factor_loading_PA_item6
measurment_table2[46,2]<- mm2_factor_loading_PA_item7
measurment_table2[47,2]<- mm2_factor_loading_PA_item8
measurment_table2[48,2]<- mm2_factor_loading_PA_item9
measurment_table2[49,2]<- mm2_factor_loading_PA_item10

measurment_table2[50,2] <- 1

measurment_table2[51,2] <- mm2_factor_loading_Neg_item1
measurment_table2[52,2]<- mm2_factor_loading_Neg_item2
measurment_table2[53,2]<- mm2_factor_loading_Neg_item3
measurment_table2[54,2]<- mm2_factor_loading_Neg_item4
measurment_table2[55,2]<- mm2_factor_loading_Neg_item5
measurment_table2[56,2]<- mm2_factor_loading_Neg_item6
measurment_table2[57,2]<- mm2_factor_loading_Neg_item7
measurment_table2[58,2]<- mm2_factor_loading_Neg_item8
measurment_table2[59,2]<- mm2_factor_loading_Neg_item9
measurment_table2[60,2]<- mm2_factor_loading_Neg_item10
...
knitr::include_graphics('Figures/Figure 2.png', dpi =300)
