...
Descriptives <- LB %>% 
  dplyr::select( Age, Gender, Religion, Ethnicity, Marital_Status,
                ...
                HLEA_lx,MEQ_Total,MEQ_Chronotype, LEBA_F1:LEBA_F5, Num_of_Coffee_Weekdays, Num_of_coffee_Weekend, Work_Shift, KSS_Alertness, OLS_Concentration_rec, OLS_Memory_rec)
...
measurment_model_1 <- constructs(
  ...
  composite("LF", multi_items("LEBA_F1_item", 3)),
...
structural_model_1 <- relationships(
  paths(from = c("L1","LF","L2","L3","L4","L5"), to = c("PSQ","SE", "PA", "NegA", "Memory","Concentration", "PT", "MA", "RT", "RI")) ,
...
mm1_factor_loading_L1_item3 <- loadings.1[loadings.1$rowname == "LEBA_F1_item3", "LF"]
...
fl_dis_val[2:17] <- lapply(fl_dis_val[2:17], FUN = function(y){as.numeric(y)})
## Discriminant validity (HTMT)
htmtdis_val <- as.data.frame(model_summary_2$validity$htmt)
htmtdis_val <- rownames_to_column(htmtdis_val)
htmtdis_val[2:17] <- lapply(htmtdis_val[2:17], FUN = function(y){as.numeric(y)})
...
mm2_factor_loading_L1_item3 <- loadings.2[loadings.2$rowname == "LEBA_F1_item3", "LF"]
